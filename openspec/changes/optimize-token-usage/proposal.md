...
- Deliver the provider-neutral benchmark framework independently from the first live provider/model execution; track provider selection, cohort setup, and the provider-attested 24-attempt result in follow-up issue #91 without claiming an efficiency win from placeholder evidence.
...
- Live provider/model selection and the first empirical corpus execution are deferred to #91; this change remains responsible for rejecting unattested or incomplete evidence.
