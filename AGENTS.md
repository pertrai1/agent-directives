...
- Bump the frontmatter `version` for every existing `directives/*.md` or `skills/*/SKILL.md` file changed in a PR. Use patch for wording or behavior-tightening, minor for new heuristics/routing/evidence coverage, and major for incompatible routing/schema/path changes. Raw deletions are rejected; deprecate with a major version bump before deletion or document an explicit policy exception. `npm run version:check` enforces this against the base branch.
